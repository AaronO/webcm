...
// include: /tmp/tmpnj6pdap8.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'webcm.data';
      var REMOTE_PACKAGE_BASE = 'webcm.data';
      ...
      async function fetchRemotePackage(packageName, packageSize) {
        ...
        // Try to fetch .gz version first
        var gzPackageName = packageName + '.gz';
        var response;
        var isGzipped = false;
        
        try {
          response = await fetch(gzPackageName);
          if (response.ok) {
            isGzipped = true;
          }
        } catch (e) {
        ...
        if (!isGzipped) {
          try {
            response = await fetch(packageName);
          } catch (e) {
        ...
        var packageData = new Uint8Array(chunks.map((c) => c.length).reduce((a, b) => a + b, 0));
        let offset = 0;
        ...
        // Decompress if gzipped and not automatically decompressed by browser
        if (isGzipped && contentEncoding !== 'gzip') {
          ...
          packageData = new Uint8Array(decompressedChunks.map((c) => c.length).reduce((a, b) => a + b, 0));
          offset = 0;
    ...
    async function runWithFS(Module) {
      ...
      async function processPackageData(arrayBuffer) {
        ...
        // Reuse the bytearray from the XHR as the source for file reads.
          ...
          Module['removeRunDependency']('datafile_webcm.data');
      }
      Module['addRunDependency']('datafile_webcm.data');
    ...
    loadPackage({"files": [{"filename": "/kernel.bin", "start": 0, "end": 17529704}, {"filename": "/rootfs.ext2", "start": 17529704, "end": 107641704}], "remote_package_size": 107641704});

  })();

// end include: /tmp/tmpnj6pdap8.js
...
function findWasmBinary() {
  ...
  if (Module['locateFile']) {
    return locateFile('webcm.wasm');
  }

  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL('webcm.wasm', import.meta.url).href;
...
var ASM_CONSTS = {
  6268: ($0, $1, $2, $3, $4) => { if (!Module._pendingResponses) Module._pendingResponses = new Map(); var headers = JSON.parse(UTF8ToString($1)); var bodyLen = $3 >>> 0; var body = bodyLen > 0 ? HEAPU8.slice($2, $2 + bodyLen) : new Uint8Array(); var headersObj = {}; for (var i = 0; i < headers.length; i++) { headersObj[headers[i][0]] = headers[i][1]; } Module._pendingResponses.set( $0, {status : $4 >>> 0, headers : headersObj, body : body}); },  
 6685: ($0, $1, $2, $3, $4, $5) => { const uid = $0; const url = UTF8ToString($1); const method = UTF8ToString($2); const headersStr = UTF8ToString($3); const bodyPtr = $4; const bodyLen = $5; const headers = {}; if (headersStr) { const lines = headersStr.split('\n'); for (const line of lines) { const idx = line.indexOf(':'); if (idx > 0) { const key = line.substring(0, idx).trim(); const value = line.substring(idx + 1).trim(); headers[key] = value; } } } let body = null; if (bodyLen > 0) { body = HEAPU8.slice(bodyPtr, bodyPtr + bodyLen); } fetch(url, {method : method, headers : headers, body : body}) .then(async response => { let respHeadersStr = ""; response.headers.forEach((value, key) => { respHeadersStr += key + ": " + value + "\n"; }); const respBody = new Uint8Array(await response.arrayBuffer()); if (!Module._forwardResponses) { Module._forwardResponses = new Map(); } Module._forwardResponses.set(uid, { status : response.status, headersStr : respHeadersStr, body : respBody }); }) .catch(error => { console.error("Forward fetch error:", error); if (!Module._forwardResponses) { Module._forwardResponses = new Map(); } Module._forwardResponses.set(uid, { status : 0, headersStr : "", body : new Uint8Array() }); }); },  
 7888: ($0) => { return (Module._forwardResponses && Module._forwardResponses.has($0)) ? 1 : 0; },  
 7971: ($0) => { return Module._forwardResponses.get($0).status; },  
 8023: ($0) => { const resp = Module._forwardResponses.get($0); const len = lengthBytesUTF8(resp.headersStr) + 1; const ptr = _malloc(len); stringToUTF8(resp.headersStr, ptr, len); return ptr; },  
 8203: ($0) => { return Module._forwardResponses.get($0).body.length; },  
 8260: ($0, $1) => { const resp = Module._forwardResponses.get($0); HEAPU8.set(resp.body, $1); },  
 8338: ($0) => { Module._forwardResponses.delete($0); },  
 8379: ($0, $1) => { if (typeof term === 'undefined') return; const end = $0 + ($1 >>> 0); const view = HEAPU8.subarray($0, end); if (!Module.__consoleTextDecoder) { Module.__consoleTextDecoder = new TextDecoder('latin1'); } const chunk = 4096; for (let i = 0; i < view.length; i += chunk) { const slice = view.subarray(i, Math.min(view.length, i + chunk)); term.write(Module.__consoleTextDecoder.decode(slice)); } }
};

// Imports from the Wasm binary.
var _vmFetch,
  _prepare_reverse_response,
  _malloc,
  _free,
  _console_queue_char,
  _console_set_size,
...
function assignWasmExports(wasmExports) {
  _vmFetch = Module['_vmFetch'] = wasmExports['vmFetch'];
  _prepare_reverse_response = Module['_prepare_reverse_response'] = wasmExports['prepare_reverse_response'];
  _malloc = Module['_malloc'] = wasmExports['malloc'];
  _free = Module['_free'] = wasmExports['free'];
  _console_queue_char = Module['_console_queue_char'] = wasmExports['console_queue_char'];
  _console_set_size = Module['_console_set_size'] = wasmExports['console_set_size'];
  _main = Module['_main'] = wasmExports['__main_argc_argv'];
  _raise = wasmExports['raise'];
  _sigaction = wasmExports['sigaction'];
